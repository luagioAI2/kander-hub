...
- Handoff is file-first: a new session must be able to resume from the card, controlled review/dispatch artifacts, Git history, and recorded verification without relying on the previous conversation. The session that starts or dispatches work remains responsible for checking the durable delivery and cleaning up; a window opening or a non-empty process is not proof that implementation completed.
- In manager mode, the current session may create, inspect, start, resume, notify, or coordinate cards without implementing their code. After a successful delegated `start`, do not poll or repeatedly inspect the card unless the user asks for progress or tracking; when asked, inspect the durable card state and evidence. This does not remove the manager's responsibility to validate delivery, review, acceptance, integration, and cleanup before completion.
