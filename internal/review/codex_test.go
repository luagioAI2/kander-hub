...
func TestUnsupportedAgentRejected(t *testing.T) {
	h := newCodexHarness(t)
	code, _, err := captureRun(t, []string{"other", h.repo, h.base, h.head, "PMQA", "目标"})
...
func TestRelativeCWDRejected(t *testing.T) {
	h := newCodexHarness(t)
	code, _, err := captureRun(t, []string{"codex", "repo", h.base, h.head, "PMQA", "目标"})
...
func TestPathOutsideGitRejected(t *testing.T) {
	...
	code, _, err := captureRun(t, []string{"codex", outside, h.base, h.head, "PMQA", "目标"})
...
func TestEmptyTaskGoalRejected(t *testing.T) {
	h := newCodexHarness(t)
	code, _, err := h.review("codex", "PMQA", "")
...
func TestUnreadableSpecRejected(t *testing.T) {
	h := newCodexHarness(t)
	code, _, err := h.review("codex", "PMQA", filepath.Join(h.root, "missing.md"))
...
func TestWindowsStyleAbsoluteIsTaskGoalOnPOSIX(t *testing.T) {
	...
	h := newCodexHarness(t)
	goal := `C:\release`
	code, _, err := h.review("codex", "PMQA", goal)
	if code != 0 {
		t.Fatalf("code=%d err=%s", code, err)
	}
...
func TestAbbreviatedSHARejected(t *testing.T) {
	h := newCodexHarness(t)
	code, _, err := captureRun(t, []string{"codex", h.repo, h.base[:8], h.head, "PMQA", "目标"})
...
func TestBaseNotAncestorRejected(t *testing.T) {
	...
	code, _, err := captureRun(t, []string{"codex", h.repo, sibling, h.head, "PMQA", "目标"})
...
func TestHEADMismatchRejected(t *testing.T) {
	h := newCodexHarness(t)
	code, _, err := captureRun(t, []string{"codex", h.repo, h.base, h.base, "PMQA", "目标"})
...
func TestSpecFileAuthoritative(t *testing.T) {
	...
	code, _, err := h.review("codex", "PMQA", spec)
	if code != 0 {
		t.Fatalf("code=%d err=%s", code, err)
	}
	prompt := readFile(t, h.stdinLog)
...
func TestIncrementalReReview(t *testing.T) {
	...
	code, _, err := captureRun(t, []string{
		"codex", h.repo, h.base, fixed, "PMQA", "确认改动正确",
...
func TestEmptyReviewedKeepsFullReview(t *testing.T) {
	h := newCodexHarness(t)
	code, _, err := captureRun(t, []string{"codex", h.repo, h.base, h.head, "PMQA", "确认改动正确", "", ""})
	if code != 0 {
		t.Fatalf("code=%d err=%s", code, err)
	}
	prompt := readFile(t, h.stdinLog)
...
func TestIncrementalRequiresLedger(t *testing.T) {
	...
	for _, ctx := range []string{"", "   ", "\n\t"} {
		_ = os.Remove(h.argvLog)
		code, _, err := captureRun(t, []string{
			"codex", h.repo, h.base, fixed, "PMQA", "确认改动正确", ctx, h.head,
		})
...
func TestTooManyArgsUsage(t *testing.T) {
	h := newCodexHarness(t)
	code, _, err := captureRun(t, []string{
		"codex", h.repo, h.base, h.head, "PMQA", "确认改动正确", "", h.base, "extra",
	})
...
func TestImplicitAgentFromConfig(t *testing.T) {
	...
	code, out, err := captureRun(t, []string{h.repo, h.base, h.head, "PMQA", "确认改动正确"})
	if code != 0 {
		t.Fatalf("code=%d err=%s", code, err)
	}
