...
func TestKanbanRulesKeepArgvPromisesAndDocumentPane(t *testing.T) {
	...
	for _, keep := range []string{
		...
		"herdr counts as started once `pane run` and any required Devin session discovery succeed; the subsequent session identity report and read-back are best-effort, failures only warn and do not enter the `LaunchFailure` tab close and card rollback path.",
	...
	for _, added := range []string{
		...
		"When `prompt_delivery.mode` is `pane`, herdr also requires prompt delivery to succeed.",
