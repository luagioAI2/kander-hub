...
Readers check the durable receipt first. Completed always returns directly. Accepted normally returns its receipt, but same-ID `notify` without a pane override may recover the original session after fresh stopped verification, as described below. Accepted `resume` without `--agent` remains receipt-only. When there is no receipt, a same-ID retry re-collects the facts: the same-ID instruction may be re-sent to the currently unique, receivable same session; only a stopped observation with valid identity permits recovering the original session. unknown, invalidated observations, missing identity, and busy do not authorize recovery. A send call that reports an error may still have delivered; keep delivery-unknown and the message file, and do not launch a recovery instance right afterwards. Once a recovery launch has attempted to hand the command to the Agent, keep the window, WINDOW, and task files even if subsequent launch, marking, or liveness verification fails; it may have already accepted the work and must not be blindly cleaned up. When a placeholder container was created but no send has been attempted, this attempt's resources may still be cleaned up. Deadline expiry returns non-zero, does not reset the deadline, and does not claim accepted.

An explicit `resume --agent` carries over the existing user-authorized takeover semantics. Taking over a non-terminal dispatch increments the execution epoch and preserves the previous epoch's original state and receipts; the original message, baseline, intent timestamps and evidence are unchanged. Prepared/delivery-unknown takeover keeps the current epoch deadline. Accepted takeover requires a stopped observation collected during this request, no older than 30 seconds, matching the card revision and SESSION/WINDOW/OWNER/STARTED_AT. It grants the new epoch a separate 120-second acceptance deadline even when the original deadline elapsed. Same-ID notify uses this same observation and transaction before recovering the original session; alive, unknown, stale and changed identity do not grant recovery. The takeover message cannot be swapped in under the same ID; when a new message is needed or an unaccepted epoch has expired, the caller first explicitly disposes of the old intent, then creates a new ID. The entry point for canceling/failing an intent is `kander dispatch cancel|fail <task-id> <dispatch-id> <dispatch-revision> <reason> [--decision <reference>]`; termination and release of the card's active binding commit together. It does not cancel, archive, or move the task card, and does not automatically authorize takeover.

The optional `execution` object in `state.json` contains `epoch`, `issued_at`, `confirm_by`, and the stopped `exit` evidence when accepted recovery renewed the budget. The transaction publishes that epoch's initial snapshot in `execution-<epoch>.json`; when retired, the snapshot is updated to retain its final state and receipts, without changing its recorded budget. Readers validate the current execution against that record. `dispatch show` returns both `intent.confirm_by` and `execution.confirm_by`; old records without `execution` fall back to the intent deadline. A pending takeover of a recovered epoch also preserves its effective deadline rather than falling back to the expired intent. Repeated retries never renew a current epoch's deadline.
...
Creation validates that the batch's current target equals the dispatch base, that the run belongs to that batch/task and is fully published, that the run's predecessor ID matches, and that each finding reference resolves in the run's `FINDINGS` or `NON_BLOCKING` array with an explicit assignment to this card. Missing references and references assigned to another card are rejected with distinct errors. Prepare stamps a read-only `section` of `FINDINGS` or `NON_BLOCKING` onto each bound reference so `dispatch show` can tell them apart; callers may omit `section` on create and on same-ID retry. A predecessor round already superseded by a successor run can no longer be dispatched. Before each actual send, the original artifacts are re-read and re-checked within the transaction that creates the send attempt, without relying on stale card paths. Cross-card copies, assignments, mappings, author dispositions, and report hashes follow the review module's structural validation. Non-blocking references never require a reviewer re-run; a disposition-only round may complete with the same delivery SHA as the prior target.

`authors` references all of this card's author artifacts that already exist for this item and its explicit finding lineage, preserving each one's author, run/finding, record ID, and relative path. When there are no prior author dispositions, they are not required to be generated in advance and `authors` may be omitted. New dispositions appearing after creation do not rewrite the original dispatch binding. A same-ID retry should omit the evidence-file and inherit the original binding directly; providing a different binding conflicts. Missing artifacts, impersonated authors, and arbitrary cross-round references are all refused. Legacy unstructured reports must first go through the review's `map-legacy` explicit mapping, then assignment; findings are not guessed from IDs mentioned in the body text.
...
The context entry point for batched liveness probing (see [Probe deadlines and cancellation](probe-deadlines.md)) provides a total probing budget (default 10 seconds), with the current epoch acceptance deadline as the parent deadline for pending delivery; accepted recovery first observes under the invocation budget because its old acceptance deadline no longer applies, then uses the newly persisted epoch deadline for delivery and validation; identity validity is checked by `ValidFor`. Readiness of the same session is checked again before delivery; alive is separate from ready, accepted, and completed. Sending and the confirmation wait share the durable deadline. Real terminal markers serve only as transport diagnostics; prompt text appearing on screen does not constitute evidence of work having started.
